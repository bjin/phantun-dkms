...
struct sk_buff *pht_build_fake_tcp_v4(const struct pht_ipv4_endpoint_pair *ep, u32 seq, u32 ack,
...
struct sk_buff *pht_build_fake_tcp_synack_v4(const struct pht_ipv4_endpoint_pair *ep, u32 seq,
                                             u32 ack) {
    return pht_build_fake_tcp_v4(ep, seq, ack, PHT_TCP_FLAG_SYN | PHT_TCP_FLAG_ACK, NULL, 0);
}

struct sk_buff *pht_build_fake_tcp_ack_v4(const struct pht_ipv4_endpoint_pair *ep, u32 seq,
                                          u32 ack) {
    return pht_build_fake_tcp_v4(ep, seq, ack, PHT_TCP_FLAG_ACK, NULL, 0);
}

struct sk_buff *pht_build_fake_tcp_ack_payload_v4(const struct pht_ipv4_endpoint_pair *ep, u32 seq,
...
struct sk_buff *pht_build_fake_tcp_rstack_v4(const struct pht_ipv4_endpoint_pair *ep, u32 seq,
                                             u32 ack) {
    return pht_build_fake_tcp_v4(ep, seq, ack, PHT_TCP_FLAG_RST | PHT_TCP_FLAG_ACK, NULL, 0);
}

struct sk_buff *pht_build_udp_v4(const struct pht_ipv4_endpoint_pair *ep, const void *payload,
                                 size_t payload_len) {
    struct sk_buff *skb;
    struct iphdr *iph;
    struct udphdr *uh;
...
int pht_tx_fake_tcp_v4(struct net *net, struct sk_buff *skb,
    ...
    flowi4_init_output(&fl4, 0, 0, 0, RT_SCOPE_UNIVERSE, IPPROTO_TCP, 0, ep->remote_addr,
                       ep->local_addr, ep->remote_port, ep->local_port, GLOBAL_ROOT_UID);
