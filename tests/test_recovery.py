...
import pytest
from helpers import (
    NS_A, NS_ADDR_A, NS_ADDR_B, NS_B, PORTS_A, PORTS_B, VETH_A, VETH_B,
    cleanup_netns_topology, ensure_netns_topology, require_guest_command,
    run_netns_scenario, spawn_netns_scenario, parse_guest_json,
    make_netns_ingress_flag_drop_probe, make_netns_output_flag_probe,
    read_module_stats,
    )
...
def test_liveness_timeout_recovers(phantun_module, vm):
    ...
    try:
        ...
        vm.run([
            'ip', 'netns', 'exec', NS_A, 'nft', 'add', 'chain', 'inet', 'filter',
            'output', '{ type filter hook output priority 10; policy accept; }'
        ])
        vm.run(['ip', 'netns', 'exec', NS_A, 'nft', 'add', 'rule', 'inet', 'filter', 'output', 'drop'])
        ...
        if rst_packets != baseline_rst:
            pytest.fail(f'expected silent liveness teardown without local RST, got {rst_packets - baseline_rst}')
...
def test_syn_isn_tie_break(phantun_module, vm):
    ...
    try:
        ...
        if final_stats['collisions_won'] - initial_stats['collisions_won'] != 1:
            pytest.fail(f"expected exactly one collision win, got stats {final_stats!r}")
...
def test_established_bare_syn_replacement(phantun_module, vm):
    ...
    try:
        ...
        vm.run([
            'ip', 'netns', 'exec', NS_B, 'nft', 'add', 'chain', 'inet', 'filter',
            'output', '{ type filter hook output priority 10; policy accept; }'
        ])
        vm.run(['ip', 'netns', 'exec', NS_B, 'nft', 'add', 'rule', 'inet', 'filter', 'output', f'ip daddr {NS_ADDR_A} drop'])
        ...
        vm.run(['ip', 'netns', 'exec', NS_B, 'nft', 'delete', 'table', 'inet', 'filter'], check=False)
