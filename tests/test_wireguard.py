...
def wait_for_handshake(vm, timeout=10):
    ...
    while time.time() < deadline:
        if (
            latest_handshake_timestamp(vm, NS_A) > 0
            and latest_handshake_timestamp(vm, NS_B) > 0
        ):
            return
        time.sleep(0.5)
...
def test_kernel_wireguard_over_phantun_uses_no_raw_udp(phantun_module, vm):
    ...
    probe_a = make_netns_output_probe(
        vm, NS_A, [(NS_ADDR_A, PORT_A, NS_ADDR_B, PORT_B)]
    )
    probe_b = make_netns_output_probe(
        vm, NS_B, [(NS_ADDR_B, PORT_B, NS_ADDR_A, PORT_A)]
    )
    ...
    try:
        ...
        if "latest handshake" not in wg_a or "latest handshake" not in wg_b:
            pytest.fail(
                "`wg show wg0` did not report a successful handshake on both peers"
            )
        if "127.0.0.1:" in wg_a or "127.0.0.1:" in wg_b:
            pytest.fail(
                "`wg show wg0` reported a loopback endpoint, which should never be required"
            )
        ...
        udp_a = probe_a.packets(
            vm, probe_comment("udp", NS_ADDR_A, PORT_A, NS_ADDR_B, PORT_B)
        )
        tcp_a = probe_a.packets(
            vm, probe_comment("tcp", NS_ADDR_A, PORT_A, NS_ADDR_B, PORT_B)
        )
        udp_b = probe_b.packets(
            vm, probe_comment("udp", NS_ADDR_B, PORT_B, NS_ADDR_A, PORT_A)
        )
        tcp_b = probe_b.packets(
            vm, probe_comment("tcp", NS_ADDR_B, PORT_B, NS_ADDR_A, PORT_A)
        )

        if udp_a != 0 or udp_b != 0:
            pytest.fail(
                f"raw UDP escaped on the WireGuard underlay path: ns_a={udp_a}, ns_b={udp_b}"
            )
        if tcp_a == 0 or tcp_b == 0:
            pytest.fail(
                f"expected translated TCP on the WireGuard underlay path, got ns_a={tcp_a}, ns_b={tcp_b}"
            )
    finally:
